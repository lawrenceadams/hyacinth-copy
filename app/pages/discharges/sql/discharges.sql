...
-- The only the name and sex fields of the mock data seem to be filled in...
SELECT TOP 100 [mrn].[mrn],
    [name].[fullname],
    [sex].[sex],
...
FROM (
    ...
    LEFT JOIN [dbo].[discharge_t03_v1] dc ON name.csn = dc.csn
    LEFT JOIN [dbo].[sex_v1] sex ON sex.csn = name.csn
    LEFT JOIN [dbo].[news2_average_last_24_hours_v1] news ON name.csn = news.csn